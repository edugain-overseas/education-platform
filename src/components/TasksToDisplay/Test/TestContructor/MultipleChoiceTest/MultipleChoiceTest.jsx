...
const MultipleChoiceTest = ({ part, setParts, handleSubmitChanges, dragHandleProps, handleDeleteQuestion }) => {
  ...
  const handleChooseCorrectAnswer = (index) => {
    setParts((prev) => {
      const updatedState = prev.map((item) => {
        if (item.id === part.id) {
          const correctCounter = item.questionAnswers.filter(
            ({ isCorrect }) => isCorrect
          ).length;
          return {
            ...item,
            questionAnswers: item.questionAnswers.map((answer, i) => {
              if (index === i) {
                return {
                  ...answer,
                  isCorrect: correctCounter === 7 ? false : !answer.isCorrect,
                };
              }
              return answer;
            }),
